...
async function drawShareImage(canvas: HTMLCanvasElement, persona: Persona, profile: UserProfile, top: LegendMatch) {
  ...
  ctx.font = `500 38px ${sans}`; ctx.fillText("나의 Riftbound 전설", W / 2, 150);
  ctx.fillStyle = "#ece4d0"; ctx.font = `600 64px ${serif}`; ctx.fillText(persona.name, W / 2, 240);

  ctx.font = `600 150px ${serif}`; ctx.fillText(championKo(l), W / 2, 520);
  ctx.fillStyle = "#c9d0e0"; ctx.font = `400 42px ${sans}`;
  ctx.fillText(`${l.officialData.champion}, ${l.officialData.title}`, W / 2, 600);

  ctx.fillStyle = "#ece4d0"; ctx.font = `600 190px ${serif}`; ctx.fillText(`${top.score}점`, W / 2, 840);
  ctx.fillStyle = "#8f9cb9"; ctx.font = `400 36px ${sans}`; ctx.fillText("추천 점수", W / 2, 895);
  ...
  stats.forEach((s, i) => {
    ...
    ctx.fillStyle = s.c; ctx.font = `600 44px ${sans}`; ctx.fillText(s.label, x, 1030);
...
export function ShareCard({ persona, profile, top, url }: { persona: Persona; profile: UserProfile; top: LegendMatch; url: string }) {
  ...
  useEffect(() => {
    ...
    if (!c) return;
    let alive = true;
    drawShareImage(c, persona, profile, top).then(() => { if (alive) setPreview(c.toDataURL("image/png")); });
    return () => { alive = false; };
  }, [persona, profile, top]);
  ...
  const save = () => {
    ...
    c.toBlob(async (blob) => {
      ...
      if (nav.canShare?.({ files: [file] })) {
        try { await nav.share({ files: [file], title: "나의 Riftbound 전설", url }); return; } catch { /* 사용자가 취소 */ }
      }
  ...
  return (
    <section className="grid gap-6 rounded-2xl border border-rim bg-deep/60 p-5 sm:grid-cols-[220px_1fr] sm:p-7" aria-labelledby="share-title">
      <canvas ref={canvasRef} width={W} height={H} className="hidden" />
      <div className="mx-auto w-[200px] sm:w-full">
        {preview ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={preview} alt={`공유 이미지: ${persona.name}, ${championKo(top.legend)} ${top.score}점`} className="w-full rounded-lg border border-rim" />
        ) : (
